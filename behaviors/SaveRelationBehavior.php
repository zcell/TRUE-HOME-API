...
class SaveRelationBehavior extends Behavior
{
                ...
                if (isset($data[$modelAttribute])) {
                    ...
                    foreach ($link as $relatedAttribute => $relatedModelAttribute) {
                        if (!isset($data[$relatedAttribute]) &&
                            in_array($relatedAttribute, $modelClass::primaryKey())) {
    ...
    protected function saveRelatedRecords(BaseActiveRecord $model, ModelEvent $event)
    {
        ...
        try {
            ...
            Yii::warning(
                get_class($e) . ' was thrown while saving related records during beforeValidate event: ' .
                $e->getMessage(),
                __METHOD__
            );
    ...
    public function afterSave()
    {
        ...
        if ($this->_relationsSaveStarted === false) {
            ...
            try {
                ...
                Yii::warning(
                    get_class($e) . ' was thrown while saving related records during afterSave event: ' .
                    $e->getMessage(),
                    __METHOD__
                );
                $this->_rollbackSavedHasOneModels();
                ...
                if (!empty($owner->{$relationName})) {
                    if ($relation->multiple === true) { // Has many relation
                        $this->_relationsToDelete =
                            ArrayHelper::merge($this->_relationsToDelete, $owner->{$relationName});
                    } else {
    ...
    public function afterDelete()
    {
        ...
        foreach ($this->_relationsToDelete as $modelToDelete) {
            try {
                ...
                Yii::warning(
                    get_class($e) . ' was thrown while deleting related records during afterDelete event: ' .
                    $e->getMessage(),
                    __METHOD__
                );
                $this->_rollbackSavedHasOneModels();
    ...
    private function _getRelationKeyName($relationName)
    {
        ...
        switch ($this->relationKeyName) {
            ...
            case self::RELATION_KEY_FORM_NAME:
                /** @var BaseActiveRecord $owner */ $owner = $this->owner;
                /** @var ActiveQuery $relation */
                $relation = $owner->getRelation($relationName);
                $modelClass = $relation->modelClass;
    ...
    public function getOldRelation($relationName)
    {
        ...
        return array_key_exists($relationName, $this->_oldRelationValue) ? $this->_oldRelationValue[$relationName] :
